...
const ContactSection = () => {
  ...
  useEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => gsap.to(el, { opacity: 1, y: 0, duration: 0.8, ease: 'power2.out', pointerEvents: 'auto' }),
  ...
  return (
    <div ref={ref} className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50">
        ...
        <div className="flex items-center gap-4">
          {CONTACT_LINKS.map(({ label, href, icon }) => (
            <a
              ...
              aria-label={label}
              className="flex items-center justify-center w-10 h-10 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400 hover:border-amber-400 hover:scale-110 transition-all duration-200 shadow-sm"
            >
              {icon}
            </a>
          ))}
